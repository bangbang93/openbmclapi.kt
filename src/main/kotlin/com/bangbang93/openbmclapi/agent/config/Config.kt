...
data class ClusterConfig(
  ...
  val flavor: ConfigFlavor =
      ConfigFlavor(runtime = "Kotlin/${KotlinVersion.CURRENT}", storage = storage)
}

data class ConfigFlavor(val runtime: String, val storage: String)

@Single fun getEnv() = applicationEnvironment { configure("application.yaml") }
...
fun getConfig(env: ApplicationEnvironment): ClusterConfig {
  ...
  fun lookupString(ktorKey: String?, dotenvKey: String?): String? =
      firstNonBlank(
  ...
  return ClusterConfig(
      clusterId = lookupString("openbmclapi.cluster.id", "CLUSTER_ID") ?: "test-cluster",
      // Default for testing
      clusterSecret = lookupString("openbmclapi.cluster.secret", "CLUSTER_SECRET") ?: "test-secret",
      // Default for testing
      clusterIp = lookupString("openbmclapi.cluster.ip", "CLUSTER_IP"),
      port = lookupString("openbmclapi.cluster.port", "CLUSTER_PORT")?.toIntOrNull() ?: 4000,
      clusterPublicPort =
          lookupString("openbmclapi.cluster.publicPort", "CLUSTER_PUBLIC_PORT")?.toIntOrNull()
              ...
              ?: 4000,
      byoc = lookupString("openbmclapi.cluster.byoc", "CLUSTER_BYOC")?.toBoolean() ?: false,
      disableAccessLog =
          lookupString("openbmclapi.cluster.disableAccessLog", "DISABLE_ACCESS_LOG")?.toBoolean()
              ?: false,
      enableUpnp =
          lookupString("openbmclapi.cluster.enableUpnp", "ENABLE_UPNP")?.toBoolean() ?: false,
      storage = lookupString("openbmclapi.storage.type", "CLUSTER_STORAGE") ?: "file",
      storageOpts =
          parseStorageOpts(lookupString("openbmclapi.storage.opts", "CLUSTER_STORAGE_OPTIONS")),
      sslKey = lookupString("openbmclapi.ssl.key", "SSL_KEY"),
      sslCert = lookupString("openbmclapi.ssl.cert", "SSL_CERT"),
      clusterBmclapi =
          lookupString("openbmclapi.cluster.bmclapi", "CLUSTER_BMCLAPI")
