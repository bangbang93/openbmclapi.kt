...
/** Alist WebDAV 存储驱动 支持从 Alist 的 WebDAV 获取 302 重定向地址并缓存 */
class AlistWebdavStorage(configMap: Map<String, String>) : WebdavStorage(configMap) {
    ...
    private val httpClient = HttpClient(CIO) { followRedirects = false }
    private val redirectUrlCache = ConcurrentHashMap<String, CachedRedirectUrl>()
    private val cacheMutex = Mutex()

    data class CachedRedirectUrl(val url: String, val expiresAt: Long)
