...
class MinioStorage(configMap: Map<String, String>) : IStorage {
  ...
  init {
    ...
    bucket =
        pathParts.firstOrNull()
            ?: throw IllegalArgumentException("MinIO bucket not specified in url path")
        ...
        withContext(Dispatchers.IO) {
          ...
          if (name != null) {
            args.extraQueryParams(
                mapOf("response-content-disposition" to "attachment; filename=\"$name\"")
            )
          }
