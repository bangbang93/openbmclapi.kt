...
class OssStorage(configMap: Map<String, String>) : IStorage {
    ...
    override suspend fun getMissingFiles(files: List<FileInfo>): List<FileInfo> =
        withContext(Dispatchers.IO) {
            ...
            try {
                var listing = client.listObjects(ListObjectsRequest(bucket).withPrefix(prefix).withMaxKeys(1000))
                ...
                while (true) {
                    ...
                    listing =
                        client.listObjects(
                            ListObjectsRequest(bucket).withPrefix(prefix).withMarker(listing.nextMarker).withMaxKeys(1000),
                        )
                }
            } catch (e: Exception) {
