...
class OssStorage(configMap: Map<String, String>) : IStorage {
    ...
    init {
        ...
        client =
            OSSClientBuilder().build(config.endpoint, config.accessKeyId, config.accessKeySecret)
    }
    ...
    override suspend fun getMissingFiles(files: List<FileInfo>): List<FileInfo> =
        withContext(Dispatchers.IO) {
            ...
            try {
                var listing =
                    client.listObjects(
                        ListObjectsRequest(bucket).withPrefix(prefix).withMaxKeys(1000)
                    )
                ...
                while (true) {
                    for (objectSummary in listing.objectSummaries) {
                        ...
                        if (file != null && file.size == objectSummary.size) {
                            this@OssStorage.files[hash] =
                                FileMetadata(objectSummary.size, objectName)
                            remoteFileMap.remove(hash)
                        }
                    }
