...
fun Application.module() {
    // Try to get config from Koin if already initialized, otherwise load it
    val config =
        try {
            GlobalContext.get().get<ClusterConfig>()
        } catch (e: IllegalStateException) {
            // Koin not initialized yet (e.g., in tests), load config directly
            loadConfig()
        }
